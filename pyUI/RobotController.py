...
tipSkillEditor = ['tipRepeat', 'tipSet', 'tipStep', 'tipTrig', 'tipTrigAngle', 'tipDelay', 'tipNote', 'tipDel',
                  'tipAdd', ]
...
class RobotController:
    ...
    def updatePortMenu(self):
        ...
        buttons = [self.frameDial.winfo_children()[i] for i in
                   [2, 3, 4]]  # [2,4] by skipping 3 was used to disable the gyro
        for b in buttons:
            b.config(state=stt)
    ...
    def popEyeColor(self):
        ...
        eyeR = self.canvasFace.create_oval(crd[0], crd[1], crd[0] + dia, crd[1] + dia, outline="#000",
                                           fill="#606060", width=2)
        eyeL = self.canvasFace.create_oval(crd[0] + dia + gap, crd[1], crd[0] + 2 * dia + gap, crd[1] + dia,
                                           outline="#000",
                                           fill="#606060", width=2)
        self.eyes = [eyeR, eyeL]
    ...
    def export(self):
        ...
        if file:
            ...
            filePathName = configDir + separation + 'SkillLibrary' + separation + self.model + separation + \
                           file.name.split('/')[-1]
